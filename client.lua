-------------------------------------------------
--╦╔═╔═╗╔╗╔╔═╗╦ ╦ -- DISCORD: @Kanoh#9824
--╠╩╗╠═╣║║║║ ║╠═╣ -- GITHUB: https://github.com/kanohesp
--╩ ╩╩ ╩╝╚╝╚═╝╩ ╩ -- DISCORD: @Kanoh#9824
-------------------------------------------------
-- Please, dont remove/change credits...
ESX = nil
...
local dist = nil
local begin = true
local zettext = "~h~~c~Pulsa ~b~[E]~c~ para iniciar un ~b~1v1"
...
AddEventHandler("updatetextclient", function(info)
    zettext = "~h~~c~1v1 por $~b~" .. info .. " ~c~1/2 ~b~[E]~c~"
    colored3d = {
        r = 252, g = 148, b = 78
    }
    inprogress = true
...
AddEventHandler("playerlost", function(amount, ped)
    notify("~h~~c~Je hebt -$~r~" .. amount .. " ~c~verloren")
    SetEntityCoords(ped, repawnlocation.x, repawnlocation.y, repawnlocation.z, 0, 0, 0, false)
end)
...
AddEventHandler("matchstarted", function()
    zettext = "~h~~c~Ya hay una wager ~b~1v1 ~c~empezada espera porfavor!"
    colored3d = {
        r = 255, g = 0, b = 0
    }
    full = true
...
AddEventHandler("resetmatch", function()
    zettext = "~h~~c~Pulsa ~b~[E]~c~ para iniciar un ~b~1v1"
    colored3d = {
        r = 0, g = 128, b = 255
    }
    full = false
